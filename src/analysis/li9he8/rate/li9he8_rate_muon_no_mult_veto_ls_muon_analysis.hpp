#ifndef JRAFNECK_ANALYSIS_LI9HE8_LI9HE8RATEMUONNOMULTVETOLSMUONANALYSIS_HPP_
#define JRAFNECK_ANALYSIS_LI9HE8_LI9HE8RATEMUONNOMULTVETOLSMUONANALYSIS_HPP_

#include "analysis/li9he8/rate/li9he8_rate_muon_analysis.hpp"
#include "selection/constants.hpp"
#include "utils/muon.hpp"

class li9he8_rate_muon_no_mult_veto_ls_muon_analysis : public li9he8_rate_muon_analysis {

public:

    using li9he8_rate_muon_analysis::li9he8_rate_muon_analysis;

    virtual ~li9he8_rate_muon_no_mult_veto_ls_muon_analysis() override = default;
    ...
    bool selection() override {
        ...
        dt_to_last_muon_result res = calculate_dt_to_last_ls_muon_with_neutron(prompt, m_nav->muons, m_nav->neutrons);
        m_dt_last_mu_with_neu = res.dt_last_mu;
        if (!res.is_set) {
            m_dt_last_mu_with_neu = timestamp{-1, 0};
        }
        res = calculate_dt_to_last_ls_muon(prompt, m_nav->muons);
        ...
        for (std::size_t k = 0ll; k < nb_radius; ++k) {
            ...
            res = calculate_dt_to_last_ls_muon_with_neutron_within_cylinder(prompt, m_nav->muons, m_nav->neutrons, radius);
            m_dt_last_mu_with_neu_radius[k] = res.dt_last_mu;
            if (!res.is_set) {
                m_dt_last_mu_with_neu_radius[k] = timestamp{-1, 0};
            }
            res = calculate_dt_to_last_ls_muon_within_cylinder(prompt, m_nav->muons, radius);
...
#endif // JRAFNECK_ANALYSIS_LI9HE8_LI9HE8RATEMUONNOMULTVETOLSMUONANALYSIS_HPP_