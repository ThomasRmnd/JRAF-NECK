#ifndef JRAFNECK_ANALYSIS_MUON_RATE_MUONRATETARGETLSMUONANALYSIS_HPP_
#define JRAFNECK_ANALYSIS_MUON_RATE_MUONRATETARGETLSMUONANALYSIS_HPP_

#include "analysis/muon/rate/muon_rate_analysis.hpp"

class muon_rate_target_ls_muon_analysis : public muon_rate_analysis {

public:

    muon_rate_target_ls_muon_analysis(const std::string& name, const std::string& filepath, const std::string& treename, const std::string& target) :
        muon_rate_analysis{name, filepath, treename},
        m_target{target}
    {}

    ~muon_rate_target_ls_muon_analysis() override = default;
    ...
    bool process() override {
        ...
        if (totq_cd < 500.0e3) return true; // high probability of water buffer muon
...
#endif // JRAFNECK_ANALYSIS_MUON_RATE_MUONRATETARGETLSMUONANALYSIS_HPP_