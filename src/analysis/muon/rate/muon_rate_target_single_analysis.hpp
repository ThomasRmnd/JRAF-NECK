#ifndef JRAFNECK_ANALYSIS_MUON_RATE_MUONRATETARGETSINGLEANALYSIS_HPP_
...
public:
    ...
    muon_rate_target_single_analysis(const std::string& name, const std::string& filepath, const std::string& treename, const std::string& target) :
        muon_rate_analysis{name, filepath, treename},
        m_target{target}
    {}

    ~muon_rate_target_single_analysis() override = default;
    ...
    bool process() override {
        ...
        if (nb_cdclassify > 1ul || nb_wpclassify > 1ul) return false;
