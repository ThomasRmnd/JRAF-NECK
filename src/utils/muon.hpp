...
inline dt_to_last_muon_result calculate_dt_to_last_ls_muon(const vertex& prompt, const std::vector<track>& muons, double qthold = 500.0e3) {
    dt_to_last_muon_result res;
    is_cd_muon_lookup cd_muons_in_event;
    cd_muons_in_event.fill(muons);
    for (const track& muon : muons) {
        if (muon.totq_cd < qthold) continue;
        if (prompt.ts < muon.ts) continue;
        if (!cd_muons_in_event[muon.ts]) continue;
        if (res.is_set && prompt.ts - muon.ts > res.dt_last_mu) continue;
        res.dt_last_mu = prompt.ts - muon.ts;
...
inline dt_to_last_muon_result calculate_dt_to_last_ls_muon_within_cylinder(const vertex& prompt, const std::vector<track>& muons, double radius, double qthold = 500.0e3) {
    dt_to_last_muon_result res;
    is_cd_muon_lookup cd_muons_in_event;
    cd_muons_in_event.fill(muons);
    for (const track& muon : muons) {
...
inline dt_to_last_muon_result calculate_dt_to_last_ls_muon_with_neutron(const vertex& prompt, const std::vector<track>& muons, const std::vector<vertex>& neutrons, double qthold = 500.0e3) {
    dt_to_last_muon_result res;
    is_cd_muon_lookup cd_muons_in_event;
    cd_muons_in_event.fill(muons);
    for (const track& muon : muons) {
...
inline dt_to_last_muon_result calculate_dt_to_last_ls_muon_with_neutron_within_cylinder(const vertex& prompt, const std::vector<track>& muons, const std::vector<vertex>& neutrons, double radius, double qthold = 500.0e3) {
    dt_to_last_muon_result res;
    is_cd_muon_lookup cd_muons_in_event;
    cd_muons_in_event.fill(muons);
    for (const track& muon : muons) {
